...
batch_norm = partial(slim.batch_norm, decay=0.95, scale=True, epsilon=1e-5, updates_collections=None)
...
def batch_norm_template(inputs, is_training, scope, moments_dims, bn_decay):
  ...
  with tf.variable_scope(scope) as sc:
    ...
    beta = tf.get_variable('beta', shape=[num_channels], initializer=tf.zeros_initializer, dtype=tf.float32, trainable=True)
    gamma = tf.get_variable('gamma', shape=[num_channels], initializer=tf.ones_initializer, dtype=tf.float32, trainable=True)
